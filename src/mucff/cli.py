...
def _parser() -> argparse.ArgumentParser:
    ...
    robustness.add_argument("--resume", action="store_true")
    return parser
...
def main() -> None:
    ...
    if args.command == "robustness":
        ...
        for path in discover_ledgers(args.data):
            ...
            task_metrics = task_output / "robustness_metrics.csv"
            if args.resume and task_metrics.is_file():
                result = pd.read_csv(task_metrics)
                print(f"reused {ledger.task_id}", flush=True)
            else:
                result = run_robustness(ledger, config, settings, args.repeats)
                result.to_csv(task_metrics, index=False)
